...
- Mochi’s existing transparent pocket hosts the panes; Cyclop’s notch shell and unrelated tools are not imported.
...
Modified upstream files: `Services/Support.swift`, `Services/ConfigStore.swift`, `Services/ShelfStore.swift`, `Services/ScreenshotFolderWatcher.swift`, `UI/Theme.swift`, `UI/SpoilerField.swift`, `UI/ShelfPane.swift`, `UI/SnippetsPane.swift`, `UI/NotesPane.swift`, `UI/MediaPane.swift`.

Additional Socius adaptations: Notes uses `PocketNoteEditor` (an owned NSTextView/NSScrollView) to avoid the legacy scrollbar gutter while retaining Cyclop’s NoteStore and autosave. Privacy dots use the light theme’s ink color. Music controls system output volume for every source. Its refreshed layout adds a playback-state bar indicator, restrained artwork scaling, and press feedback with Reduce Motion support; it does not sample audio. SnippetInput uses a native text field with a stable editing baseline. Clipboard and Snippets have shorter pocket pages. These changes are recorded in the integration patch where upstream files were modified.
